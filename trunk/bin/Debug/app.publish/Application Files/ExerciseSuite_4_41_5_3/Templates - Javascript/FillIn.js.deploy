...
for (i = 0;  i < OrigQuestions.length;  i++) {
  for (j = 0;  j < OrigQuestions[i][0];  j++) {
    ...
    QuestionList[i] = QuestionList[i].replace('<input' + j + '>', '<input type="text" style="display:inline;" size="' + AnswerList[iTotalInputNum].length + '"><span id="staticAnswer' + iTotalInputNum + '" style="display:none;font-weight:bold;">' + AnswerList[iTotalInputNum] + '</span>');
    iTotalInputNum++;
  }
}
